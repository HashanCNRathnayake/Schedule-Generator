    ...
    <script>
        ...
        function renderCourseDetails(data, btn) {
            return `
                ...
                <p><b>ID:</b> ${btn.dataset.id}<br><b>Code:</b> ${btn.dataset.code}</p>
                ...
                <p><b>SOC:</b> ${data.data.master_learning_paces?.[0]?.pace || '—'}</p>
                ...
                <table class="table table-bordered table-sm">
                    <tr><th>No</th><th>Type</th><th>Short</th><th>Details</th><th>Duration</th><th>Day</th></tr>
                    ${data.data.modules?.[0]?.session_plans?.map((s, i) => `
                        <tr>
                            <td>${i + 1}</td>
                            <td>${s.session_type_name}</td>
                            <td>${s.session_short_name}</td>
                ...
                <h4>Faculty</h4>
                <ul>
                    ${(data.data.master_learning_faculties || []).map(f => `<li>${f.faculty}</li>`).join('')}
                </ul>
            `;
        }
    </script>
