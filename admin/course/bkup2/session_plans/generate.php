<?php
// admin/session_plans/generate.php
...
require __DIR__ . '/../lib/schedule_lib.php';
...
// state
$selected = [
    'course_id'     => trim($_GET['course_id'] ?? $_POST['course_id'] ?? ''),
    'course_code'   => trim($_GET['course_code'] ?? $_POST['course_code'] ?? ''),
    'module_code'   => trim($_GET['module_code'] ?? $_POST['module_code'] ?? ''),
    'learning_mode' => trim($_GET['learning_mode'] ?? $_POST['learning_mode'] ?? ''),
];
...
<body class="py-4">
        ...
        <form method="post" class="mb-4">
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <a class="btn btn-outline-secondary" href="../session_templates/master.php">Create/Edit Templates</a>
                </div>
            </div>
        </form>
    ...
    <script>
        ...
        searchInput?.addEventListener('input', async () => {
            ...
            const res = await fetch('../course/search_courses.php?q=' + encodeURIComponent(q));
    ...
    `).join('');
        ...
        resultsBox?.addEventListener('click', async e => {
            ...
            const res = await fetch('../course/get_course_details.php?id=' + btn.dataset.id);
