...
if (isPost('clearCsv')) {
    unset($_SESSION['grid'], $_SESSION['generated']);
...
function csv_to_rows($tmpPath)
{
    ...
    while (($cols = fgetcsv($fh)) !== false) {
        ...
        // Detect the header row (Session No ... Duration Hrs)
        if (!$headerFound) {
            ...
            continue; // keep skipping until header row
        }

        // Only pick first 4 columns (A-D)
        $c0 = $cols[0] ?? '';
        $c1 = $cols[1] ?? '';
        $c2 = $cols[2] ?? '';
        $c3 = $cols[3] ?? '';
...
<body class="py-4">
    <div class="container">
        ...
        <div id="pdfArea">
            <div class="card mb-4  <?php if (!$grid) echo 'd-none'; ?> ">
                <div class="card-body">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="flex d-flex justify-content-between align-items-center mb-3">
                            ...
                            <table class="table table-sm table-bordered align-middle text-nowrap">
                                ...
                                <tbody>
                                    ...
                                    if (!$show && $grid) {
                                        ...
                                        foreach ($show as $i => $r) {
                                            echo '<tr>
                                                    <!-- Read-only values (Session No, Type, Details, Duration) -->
                                                    ...
                                                    <!-- Editable Faculty -->
                                                    <td><input name="rows[' . $i . '][faculty]" class="form-control form-control-sm"
                                                            value="' . h($r['faculty']) . '"></td>

                                                    <!-- Editable Date -->
                                                    <td><input type="date" name="rows[' . $i . '][date]" class="form-control form-control-sm date-input"
                                                            value="' . h($r['date']) . '" data-day-target="day-' . $i . '"></td>

                                                    <!-- Auto Day (readonly, updated via JS) -->
                                                    <td><input id="day-' . $i . '" name="rows[' . $i . '][day]"
                                                            class="form-control form-control-sm" value="' . h($r['day']) . '" readonly></td>

                                                    <!-- Editable Time -->
                                                    <td><input name="rows[' . $i . '][time]" class="form-control form-control-sm"
                                                            value="' . h($r['time']) . '"></td>
                                                </tr>';
                                        }
                                    } else {
